        ...
        if (war_obj.attackers.includes(actual_ot_user_id) || war_obj.defenders.includes(actual_ot_user_id)) {
          ...
          if (war_obj[friendly_side].includes(actual_id)) {
            ...
            if (war_obj[`${friendly_side}_war_leader`] != actual_ot_user_id) {
              ...
              var political_capital_penalty = returnSafeNumber(war_obj[`${friendly_side}_war_leader_bids`])*config.defines.diplomacy.war_leader_bid_penalty_cost;

              var pc_cost = config.defines.diplomacy.war_leader_bid_cost + political_capital_penalty;
