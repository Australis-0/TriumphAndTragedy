        ...
        switch (map_name) {
          case "supply":
            ...
            for (var i = 0; i < 110; i+= 10) {
              var local_index = i;
              if (local_index == 100) i--; //Render 99

              ctx.fillStyle = RGBToHex(...config.defines.map.scalar_gradient[local_index]);
              ...
              ctx.fillStyle = "#ffffff";
              ctx.fillText(truncateString(`${maximum_supply_limit*(local_index/100)}`, 15), config.defines.map.map_label_coords[0] + 50 + i*320, config.defines.map.map_label_coords[1] + 47 + x*40);
            }
    ...
    switch (map_name) {
      ...
      case "supply":
        ...
        if (maximum_supply_limit == 0)
          maximum_supply_limit = 1;

        //Shade in province supply limit based on % from 0 to maximum_supply_limit
        for (var i = 0; i < all_provinces.length; i++) {
          var local_province = main.provinces[all_provinces[i]];
          var local_supply = returnSafeNumber(local_province.supply_limit);
